...
class machinelearn:
    ...
    def stat_array(self, modeldata):
        ...
        def list_to_string(array):
        	...
        	return sstring
        yt_s = list_to_string(y_test)
        yp_s = list_to_string(y_pred)
    ...
    def rfrmodel(self, test, xy, t):
        ...
        y_pred = pd.DataFrame(rfrmodel.predict(X_test),columns=['yield'],index=test.index)
    ...
    def merfmodel(self, train, test, xy, mi):
        ...
        y_pred = pd.DataFrame(y_predarray, columns=['yield'], index=test.index)
        #collect returned values into dictionary
    ...
    def xgbreg(self, train, test, xy):
        import xgboost as xgb
        X_test = xy.get('X_test')
        y_test = xy.get('y_test')
        X_train = xy.get('X_train')
        y_train = xy.get('y_train')
        dvalid = xgb.DMatrix(X_test, y_test) #didn't work when ran with XGBRegressor, switched to DMatrix
        dtrain = xgb.DMatrix(X_train, y_train)

        params = {'objective':'reg:squarederror','booster':'gbtree','num_parallel_tree':26}
...
def input_extraction(feature_names, percentiles, mon=None):
    ...
    csv = pd.read_csv('~/Desktop/code/nasaharvest/sp24_2/kenya_maize_a3b.csv')
    if mon==None:
        csv2 = csv[csv['crop_cal'] != 0]
    if mon != None:
        #exclusion of data is specifically for testing data only
        test = pd.read_csv('~/Desktop/code/nasaharvest/sp24_2/subseasonal/subseasonal/kenya_maize_2019.csv')
        #test = csv3[csv3.Year.isin([2019,2023])]
        test['date']=pd.to_datetime(test['time'])
        test['month'] = pd.DatetimeIndex(test['date']).month
        csv2 = test[test['month']<mon]
    df = csv2[['ADM1_NAME', 'Year', 'yield', 'ndvi', 'tmax', 'tmin', 'esi', 'cmax','cmin','fldas']]
    ...
    if percentiles == 'no':
        ...
        df_full = df_ml[include].reset_index()
...
#groupshead = groups[0:5,:]
    ...
    #print(df_full)
    df_full = df_full.query('2000 < Year < 2023') #modify frame
